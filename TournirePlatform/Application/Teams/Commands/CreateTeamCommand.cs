...
{
    public async Task<Result<Team, TeamException>> Handle(CreateTeamCommand request, CancellationToken cancellationToken)
    {
        ...
        return await existingTeam.Match(
            ...
            async () => await CreateEntity(request.Name, request.Icon, request.MatchCount, request.WinCount, cancellationToken));
    }
    
    private async Task<Result<Team, TeamException>> CreateEntity(string name, byte[]? icon, int matchCount, int winCount,   CancellationToken cancellationToken)
    {
        try
        {
            var entity = new Team(
